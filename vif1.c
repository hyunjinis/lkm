...
static ssize_t vif_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
	...
	if(!strcmp(filename, "image_size"))
        {
                vif->image_size = value;
...
//#if 0
	...
	else if(!strcmp(filename, "image_size")){
                len = sprintf(temp_buf, "%u\n", vif->image_size);
                if(copy_to_user(buf, temp_buf,strlen(temp_buf)))
                        printk("MINKOO: copy fail\n");
                goto proc_out;
...
void new_vif(struct net_bridge_port *p){
	...
	proc_vif[idx].file[0] = proc_create_data("min_credit",0666, proc_vif[idx].dir, &vif_opt, vif);
	if (!proc_vif[idx].file[0]) {
                printk(KERN_ERR "MINKOO: Failed to create min_credit file for vif%d\n", vif->id);
                return;
        }

	proc_vif[idx].file[1] = proc_create_data("max_credit",0666, proc_vif[idx].dir, &vif_opt, vif);
	proc_vif[idx].file[2] = proc_create_data("weight",0666, proc_vif[idx].dir, &vif_opt, vif);
	proc_vif[idx].file[3] = proc_create_data("remaining_credit",0666, proc_vif[idx].dir, &vif_opt, vif);
	proc_vif[idx].file[4] = proc_create_data("used_credit",0666, proc_vif[idx].dir, &vif_opt, vif);
	//modified
	proc_vif[idx].file[5] = proc_create_data("image_size", 0666, proc_vif[idx].dir, &vif_opt, vif);
	//****
...
void del_vif(struct net_bridge_port *p){
	...
	//modified
	remove_proc_entry("image_size", proc_vif[idx].dir);
	//****
...
static void __exit vif_exit(void)
{
	...
	if(&active_vif_list == NULL || list_empty(&active_vif_list))
                return;
